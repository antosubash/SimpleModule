                ...
                async context =>
                {
                    try
                    {
                        ...
                        var publishResponse = await client.PostAsync(
                            $"/api/pagebuilder/{page.Id}/publish",
                            null
                        );
                        if (!publishResponse.IsSuccessStatusCode)
                            return Response.Fail(
                                statusCode: ((int)publishResponse.StatusCode).ToString(
                                    System.Globalization.CultureInfo.InvariantCulture
                                )
                            );
                        ...
                        var unpublishResponse = await client.PostAsync(
                            $"/api/pagebuilder/{page.Id}/unpublish",
                            null
                        );
                        if (!unpublishResponse.IsSuccessStatusCode)
                            return Response.Fail(
                                statusCode: ((int)unpublishResponse.StatusCode).ToString(
                                    System.Globalization.CultureInfo.InvariantCulture
                                )
                            );
                        ...
                        if (!tagsResponse.IsSuccessStatusCode)
                            return Response.Fail(
                                statusCode: ((int)tagsResponse.StatusCode).ToString(
                                    System.Globalization.CultureInfo.InvariantCulture
                                )
                            );
                        ...
                        if (!templatesResponse.IsSuccessStatusCode)
                            return Response.Fail(
                                statusCode: ((int)templatesResponse.StatusCode).ToString(
                                    System.Globalization.CultureInfo.InvariantCulture
                                )
                            );
